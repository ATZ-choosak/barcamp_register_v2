...
function UserCard({ user, update_status }) {
  ...
  return (
    <div className="p-4 bg-white w-full rounded-lg shadow-md relative">
      <div className={clsx("left-0 top-0 z-10 w-full h-screen bg-black/50 flex items-center justify-center" , openSlip ? "fixed" : "hidden")}>
        <div className="max-w-md">
          <div className="w-full flex justify-end py-2">
          <button onClick={() => setOpenSlip(false)}>
            <svg
          ...
          <img src={config.apiPrefix + "/" + user.slip} />
        </div>
      </div>
      <p
        className={clsx(
